...
const covertToTHB = (value, selected) => {
  switch (selected) {
    case "THB":
      return 1 * parseFloat(value);
    case "USD":
      return 32.86 * parseFloat(value);
    case "EUR":
      return 38.82 * parseFloat(value);
    case "JPY":
      return 0.30 * parseFloat(value);
    case "CNY":
      return 5.07 * parseFloat(value);
    case "MXN":
      return 1.64 * parseFloat(value);
    case "SGD":
      return 24.46 * parseFloat(value);
    case "HKD":
      return 4.23 * parseFloat(value);
    case "KRW":
      return 0.029 * parseFloat(value);
  }
}

const covertTHBTo = (value, selected) => {
  if (value == "0") {
    return "0";
  }
  switch (selected) {
    ...
    case "USD":
      return parseFloat(value) / 32.86;
    case "EUR":
      return parseFloat(value) / 38.82;
    case "JPY":
      return parseFloat(value) / 0.30;
    case "CNY":
      return parseFloat(value) / 5.07;
    case "MXN":
      return parseFloat(value) / 1.64;
    case "SGD":
      return parseFloat(value) / 24.46;
    case "HKD":
      return parseFloat(value) / 4.23;
    case "KRW":
      return parseFloat(value) / 0.029;
  }
}
...
export default function Currency({ navigation }) {
  ...
  const [firstSelected, setFirstSelected] = useState("THB");
  const [secondSelected, setSecondSelected] = useState("USD");
  const [firstValue, setFirstValue] = useState("0");
  const [secondValue, setSecondValue] = useState("0");

  useEffect(() => {
    let firstValueToTHB = covertToTHB(firstValue, firstSelected);
    let secondValueToTHB = covertTHBTo(firstValueToTHB, secondSelected);

    if (secondValueToTHB.toString().length > 12) {
      setSecondValue(secondValueToTHB.toString().substring(0, 12));
    } else {
      setSecondValue(secondValueToTHB);
    }
      ...
      <View className="flex flex-col w-full flex-1">
        <View className="bg-black w-full pt-20 pb-4 flex flex-col">
          ...
          <View className="px-5">
            ...
            <Text className="text-slate-400">{covertTHBTo(covertToTHB(1, firstSelected), firstSelected)} {firstSelected} = {covertTHBTo(covertToTHB(1, firstSelected), secondSelected)} {secondSelected}</Text>
          </View>
        </View>
