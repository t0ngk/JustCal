...
const covertToGrams = (value, selected) => {
  switch (selected) {
    case "Grams":
    ...
    case "Pounds":
      return 453.592 * parseFloat(value);
    case "Ounces":
      return 28.3495 * parseFloat(value);
    case "Stones":
      return 6350.29 * parseFloat(value);
    case "Tons":
      return 907184 * parseFloat(value);
  }
}

const covertGramsTo = (value, selected) => {
  if (value == "0") {
    return "0";
  }
  switch (selected) {
...
export default function Weight({ navigation }) {
  ...
  const [firstSelected, setFirstSelected] = useState("Grams");
  const [secondSelected, setSecondSelected] = useState("Kilograms");
  const [firstValue, setFirstValue] = useState("0");
  const [secondValue, setSecondValue] = useState("0");

  useEffect(() => {
    let firstValueToGrams = covertToGrams(firstValue, firstSelected);
    let secondValueToGrams = covertGramsTo(firstValueToGrams, secondSelected);

    if (secondValueToGrams.toString().length > 12) {
      setSecondValue(secondValueToGrams.toString().substring(0, 12));
    } else {
      setSecondValue(secondValueToGrams);
    }
      ...
      <View className="flex flex-col w-full flex-1">
        <View className="bg-black w-full pt-20 pb-4 flex flex-col">
          ...
          <View className="px-5">
            ...
            <Text className="text-slate-400">{covertGramsTo(covertToGrams(1, firstSelected), firstSelected)} {firstSelected} = {covertGramsTo(covertToGrams(1, firstSelected), secondSelected)} {secondSelected}</Text>
          </View>
        </View>
